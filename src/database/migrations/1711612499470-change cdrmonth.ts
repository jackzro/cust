...
export class changeCdrmonth1711612499470 implements MigrationInterface {
    name = 'changeCdrmonth1711612499470'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`cdrmonths\` ADD \`tahun\` varchar(255) NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`cdrmonths\` ADD \`provider\` varchar(255) NOT NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`cdrmonths\` ADD CONSTRAINT \`FK_e7982f1c4b62bb80b1aeeeb2bc7\` FOREIGN KEY (\`customersId\`) REFERENCES \`customers\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`cdrmonths\` DROP COLUMN \`provider\``);
        await queryRunner.query(`ALTER TABLE \`cdrmonths\` DROP COLUMN \`tahun\``);
    }
